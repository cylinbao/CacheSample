...
def inference(model, g, inputs, batch_size, device):
    model.eval()
    t0 = time.time()
    with th.no_grad():
        pred = model.inference(g, inputs, batch_size, device)
...
def run(args, device, data):
    ...
    if args.inference:
        ...
        for evt in events:
            if evt.key == "GSpMM":
                avg_t = evt.cuda_time_total/num_run/1000
        print("Avg GSpMM CUDA kernel time (ms) per run: {:.3f}".format(avg_t))
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--ogb-dir', type=str, default='/home/ubuntu/.ogb')
    ...
    if args.dataset == 'reddit':
        ...
        g, n_classes = load_ogb('ogbn-products', args.ogb_dir)
    else:
