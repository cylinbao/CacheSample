...
def gen_model(args):
    if args.cache_sample:
        model = GCN(
                    ...
                    F.relu, args.dropout, norm="none"
                )
    else:
        model = GCN(
                    in_feats, args.n_hidden, n_classes, args.n_layers, 
                    F.relu, args.dropout, norm="right"
                )
...
def run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, n_running, name_base):
    ...
    return best_val_roc, best_test_roc
...
def main():
    ...
    # load data
    home = os.getenv("HOME")
    dataset = PygNodePropPredDataset(name='ogbn-proteins', root=home + "/.ogb",
    ...
    name_base = "gcn_proteins_{}_layer_{}_hidden".format(
                    args.n_layers, args.n_hidden)
    if args.train:
        for i in range(args.n_runs):
            val_acc, test_acc = run(args, graph, labels, train_idx, val_idx, test_idx, evaluator, i, name_base)
    ...
    if args.inference:
        test_acc = run_eval(args, graph, labels, train_idx, val_idx, test_idx, evaluator, name_base)
    ...
    if args.acc_analysis:
        ...
        # for i in [2]:
            ...
            test_acc = run_eval(args, _graph, labels, train_idx, val_idx, test_idx, evaluator, name_base)
            accs.append(test_acc)

        # with open("./log/" + "proteins_rand_sample_acc.csv", "w") as f:
